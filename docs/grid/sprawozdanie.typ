...
Kustomize to narzędzie do deklaratywnego zarządzania konfiguracją Kubernetes, zintegrowane z kubectl od wersji 1.14 @kustomizeKubectl. Umożliwia ono dostosowywanie manifestów YAML bez ich bezpośredniej modyfikacji, zamiast tego stosując nakładki (overlays) i transformacje.
...
Multi-stage builds to technika budowania obrazów kontenerowych polegająca na wykorzystaniu wielu etapów (stages) w jednym Dockerfile. Każdy etap może bazować na innym obrazie bazowym i służyć konkretnemu celowi (instalacja zależności, kompilacja kodu, przygotowanie środowiska runtime). Finalny obraz zawiera tylko niezbędne artefakty z poprzednich etapów, co drastycznie redukuje jego rozmiar @kubernetesUpRunning.
...
*Build-time ARG dla NEXT_PUBLIC_API_URL* - Next.js wymaga, aby zmienne środowiskowe z prefiksem `NEXT_PUBLIC_` były dostępne podczas budowania (build-time), ponieważ są wbudowywane w kod JavaScript przesyłany do przeglądarki @nextEnvVars. Dockerfile wykorzystuje mechanizm ARG do przekazania URL API podczas budowania obrazu: `docker build --build-arg NEXT_PUBLIC_API_URL=http://api.okane.local/`.
...
*Next.js standalone output* - Next.js w wersji 12+ oferuje tryb standalone, który generuje minimalną, samodzielną wersję aplikacji zawierającą tylko niezbędne zależności @nextOutputStandalone. W przeciwieństwie do standardowego buildu, który kopiuje całe node_modules, standalone output zawiera tylko wykorzystywane zależności, co znacząco redukuje rozmiar obrazu. Konfiguracja w next.config.js: `output: 'standalone'`.
...
Minikube oferuje komendę `image build`, która buduje obrazy kontenerowe bezpośrednio w środowisku klastra. Dzięki temu nie ma potrzeby pushowania obrazów do zewnętrznego registry - są one dostępne lokalnie w węźle Minikube @minikubePushingImages.
...
Katalog `overlays/prod` zawiera plik `kustomization.yaml`, który referencjuje zasoby z base i nadpisuje specyficzne wartości dla środowiska produkcyjnego (np. tagi obrazów, liczba replik, zmienne środowiskowe).
...
*ClusterIP (domyślny) dla komunikacji wewnętrznej* - Service typu ClusterIP otrzymuje wirtualny IP dostępny tylko wewnątrz klastra. To domyślny i najbardziej popularny typ Service, używany do komunikacji między komponentami aplikacji @k8sClusterIP.
...
*Persistentne dane przez PV/PVC (1Gi, hostPath)* - StatefulSet używa PersistentVolumeClaim do zapewnienia trwałości danych bazy danych. W Kubernetes StatefulSet jest przeznaczony do zarządzania aplikacjami stanowymi wymagającymi trwałego storage i stabilnej tożsamości podów, takimi jak bazy danych @k8sStatefulSet.
...
*LoadBalancer service dla ingress-nginx* - Aby `minikube tunnel` mógł wystawić Ingress na portach 80/443 na hoście, Ingress Controller potrzebuje Service typu LoadBalancer. Minikube addon automatycznie tworzy taki Service, a `minikube tunnel` odpowiada za routing ruchu z hosta do tego LoadBalancera @minikubeTunnel.
